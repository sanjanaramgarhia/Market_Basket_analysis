...
def show_login_page():
    ...
    with loginSection:
        ...
        if st.button("Login"):
            # Allow any username and location (both fields must be filled)
            if username.strip() and location.strip():
                st.success(f"😊 Welcome, {username} from {location}!")
                # Store user info in session state
                st.session_state["username"] = username
                st.session_state["location"] = location
                LoggedIn_Clicked()
            else:
                st.error("❌ Please enter both username and location")
